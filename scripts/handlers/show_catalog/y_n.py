...
from sql.bd_delit import yes_delit
...
async def today_delit(message: Message, state: FSMContext) -> None:
    ...
    await message.reply(f'Ты хочешь удалить {message.text}', reply_markup=yes_or_no())
...
async def today_delit(message: Message, state: FSMContext) -> None:
    ...
    await message.reply('Задача успешно удалена! Востановлению не подлежит :)', reply_markup=again())
    await state.clear()

@router.message(UserState.chose_yes_or_no, F.text=="НЕТ! Я еще подумаю!")
async def today_delit(message: Message, state: FSMContext) -> None:
    await message.reply('Хорошо, я не буду ее удалять :)', reply_markup=again())
    await state.clear()
